...
class Student:  # Defines the student class with attributes & methods for the student
    ...
    def determine_pass_fail(
        self
    ):  # Determines whether student has passed or failed based on average grade
...
class GradeTrackerGUI:
    # Defines the GradeTrackerGUI class, which is the main GUI for the grade tracker system.
    def __init__(self, root):
        ...
        self.main_frame = ttk.Frame(
            root, padding="10")  # Creates a GUI main frame with padding
        self.main_frame.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        # Packs the main frame into the root window, with padding and sticky edges
        ...
        ttk.Label(self.main_frame,
                  ...
                  font=('Helvetica', 16, 'bold')).grid(row=0,
                                                       ...
                                                       columnspan=2,
                                                       pady=10)
        # Creates the main label for the GUI main menu

        ttk.Button(self.main_frame,
                   text="Add New Student",
                   command=self.show_add_student).grid(row=1,
                                                       column=0,
                                                       columnspan=2,
                                                       pady=5,
                                                       sticky=tk.EW)
        ttk.Button(self.main_frame,
                   text="Enter Grades",
                   command=self.show_enter_grades).grid(row=2,
                                                        column=0,
                                                        columnspan=2,
                                                        pady=5,
                                                        sticky=tk.EW)
        ttk.Button(self.main_frame,
                   text="View Reports",
                   command=self.show_reports).grid(row=3,
                                                   column=0,
                                                   columnspan=2,
                                                   pady=5,
                                                   sticky=tk.EW)
        # Creates the buttons for the main menu, with commands to open the respective dialogs

    def show_add_student(
            self):  # Defines this method to show the add student dialog
        dialog = tk.Toplevel(self.root)  # Creates a new dialog window
        dialog.title("Add New Student")
        dialog.geometry("300x300")

        ttk.Label(dialog, text="First Name:").grid(row=0,
                                                   column=0,
                                                   pady=5,
                                                   padx=5)
        # Creates labels & entry fields for the student's details
        name_entry = ttk.Entry(
            dialog)  # Creates an entry field for the student's name
        name_entry.grid(row=0, column=1, pady=5, padx=5)
        # Packs the entry field into the dialog window

        ttk.Label(dialog, text="Surname:").grid(row=1,
                                                column=0,
                                                pady=5,
                                                padx=5)
        ...
        ttk.Label(dialog, text="Birth Year:").grid(row=2,
                                                   column=0,
                                                   pady=5,
                                                   padx=5)
        ...
        ttk.Label(dialog, text="Number of Subjects:").grid(row=3,
                                                           column=0,
                                                           pady=5,
                                                           padx=5)
        ...
        ttk.Label(dialog, text="Honours Student:").grid(row=4,
                                                        column=0,
                                                        pady=5,
                                                        padx=5)
        ...
        def submit(
        ):  # Defines the submit method to add the student to the grade tracker system
            ...
            # Validate birth year
            if not (birth_year.isdigit() and birth_year >= '1945'
                    and birth_year <= '2023'):
                messagebox.showerror(
                    "Error",
                    "Birth year must be a number between 1945 and 2023!")
                return
            ...
            try:
                ...
                if not (1 <= subject_count <= 14):
                    messagebox.showerror(
                        "Error", "Subject count must be between 1 and 14!")
                    return
            except ValueError:
                messagebox.showerror("Error",
                                     "Subject count must be a number!")
                return

            student_id = name[0:3] + surname[0:3] + birth_year
            self.tracker.add_student(name, student_id, surname, birth_year,
                                     honours_var.get(), subject_count)
            messagebox.showinfo(
                "Success",
                f"Student added successfully!\nStudent ID: {student_id}")
            dialog.destroy()

        ttk.Button(dialog, text="Add Student",
                   command=submit).grid(row=5, column=0, columnspan=2, pady=10)
    ...
    def show_enter_grades(self):
        ...
        ttk.Label(id_dialog, text="Student ID:").grid(row=0,
                                                      column=0,
                                                      pady=5,
                                                      padx=5)
        ...
        ttk.Button(id_dialog, text="Continue",
                   command=check_id).grid(row=1,
                                          column=0,
                                          columnspan=2,
                                          pady=10)
    ...
    def show_grade_entry_form(self, student_id):
        ...
        for i in range(subject_count):
            ttk.Label(grades_frame, text=f"Grade {i+1}:").grid(row=i,
                                                               column=0,
                                                               pady=10,
                                                               padx=5,
                                                               sticky='e')
        ...
        ttk.Button(dialog, text="Submit Grades",
                   command=submit).grid(row=1, column=0, pady=20, sticky='ew')
